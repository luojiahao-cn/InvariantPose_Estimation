...
% 初始化各算法初值
p_init_lm   = start_p + 0.01*randn(3,1);
theta_init_lm = theta_true + 0.1*randn(3,1);
R_init_lm   = MatrixExp3(VecToso3(theta_init_lm));

p_init_elm  = p_init_lm;
theta_init_elm = theta_init_lm;
R_init_elm  = R_init_lm;

p_init_ours = p_init_lm;
R_init_ours = R_init_lm;

p_init_Rlm  = p_init_lm;
theta_init_Rlm = theta_init_lm;
R_init_Rlm  = R_init_lm;
...
for idx = 1:num_points
    ...
    [p_lm, R_lm, stats_lm] = estimate_pose_lm( ...
        b_total, d_list, m_pos, m_hat, m_norm, theta_init_lm, p_init_lm, options, lb_p, ub_p );
    t_lm = toc(t_lm_start);

    % ELM
    t_elm_start = tic;
    [p_elm, R_elm, stats_elm] = estimate_pose_elm( ...
        b_total, d_list, m_pos, m_hat, m_norm, theta_init_elm, p_init_elm, options, lb_p, ub_p );
    t_elm = toc(t_elm_start);

    % 所提算法
    t_ours_start = tic;
    [p_ours, R_ours, stats_ours] = estimate_pose_ours( ...
        b_total, d_list, m_pos, m_hat, m_norm, p_init_ours, R_init_ours, R_true, p_true, options, lb_p, ub_p );
    t_ours = toc(t_ours_start);

    % Rlm
    t_Rlm_start = tic;
    [p_Rlm, R_Rlm, stats_Rlm] = estimate_R_lm( ...
        b_total, d_list, m_pos, m_hat, m_norm, theta_init_Rlm, p_ours, options );
    ...
    % 更新各算法初值为本次结果
    p_init_lm   = p_lm;
    theta_init_lm = so3ToVec(MatrixLog3(R_lm));
    R_init_lm   = R_lm;

    p_init_elm  = p_elm;
    theta_init_elm = so3ToVec(MatrixLog3(R_elm));
    R_init_elm  = R_elm;
    ...
    % p_init_Rlm  = p_ours;
    theta_init_Rlm = so3ToVec(MatrixLog3(R_Rlm));
    R_init_Rlm  = R_Rlm;
end
